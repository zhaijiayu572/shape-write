...
export function write(fields, data, filename, encoding = 'gb18030') {
  ...
  let buffer = new ArrayBuffer(
    32 + fieldDescLength + bytePerRecord * data.length + 1
  )
  ...
  _.forEach(fields, (f, i) => {
    ...
    if (f.type === 'N')
      view.setInt8(32 + i * 32 + 17, f.precision ? f.precision : 0)
    ...
    _.forEach(fields, (f) => {
      ...
      switch (f.type) {
        case 'L':
          view.setUint8(
            offset,
            val === true ? 0x54 : val === false ? 0x46 : 0x3f
          )
          offset++
          break
        case 'D':
...
function ensureFieldSize(fields) {
  fields.forEach((field) => {
    switch (field.type) {
      case 'C':
        field.size = Math.min(
          field.size && field.size > 0 ? field.size : 10,
          255
        )
        break
      case 'N':
        ...
        field.precision = Math.min(
          field.precision && field.precision >= 0 ? field.precision : 0,
          19
        )
        break
      case 'D':
